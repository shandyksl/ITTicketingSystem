...
namespace luvo.erp.ngerp1
{
    public class CModule
    {
        public static string mastercurr = "MYR";
        public static int qtydecimalpoint = 0;
        public static int amtdecimalpoint = 2;
        public static int roundamtdecimalpoint = 5;
        public static int roundqtydecimalpoint = 5;
        public static string cc = "NTMP";
        public static string ticketNo { get; set; }
        public static string ticketNoStatus { get; set; }

        public static string enhancementticketNo { get; set; }
        public static string enhancementticketNoStatus { get; set; }



        public static string un { get; set; }
        public static string firstvalue { get; set; }

        public static DataTable tblincident { get; set; }


        public static int len_ctno = 10, len_vcno = 5, len_serno = 8, len_rcnno = 8, len_sercusno = 4, len_dncno = 8, len_dndno = 10;
        public static string logintype = "NSM-EXT";
        //frmVF
        public static string vf2_keyw = "", vf2_sts = "APP";
        public static bool vf2_vc = true, vf2_nm = true, vf2_hp = true;


        public static SqlConnection cnn = new SqlConnection(@"Server=localhost\SQLEXPRESS;Database=ITTicketing;Integrated Security=True;");


        public static void UpdLog(string cc, string fn1, string fn2, string fid1, string fid2, string act, string pn1, string pn2, string rmk, string un)
        {
            //DateTime dt1 = cl.GetTimeInSQLServer(), dt2 = DateTime.Now;
            //TimeSpan ts = dt2.Subtract(dt1);
            //string env = "APPVER:" + ver + "\r\n";
            //env += "MACHINENAME:" + System.Environment.MachineName + "\r\n";
            //env += "OSVER:" + System.Environment.OSVersion + "\r\n";
            //env += "WINUSER:" + System.Environment.UserName + "\r\n";
            //env += "TIMEDIFF(S):" + Math.Round(ts.TotalSeconds, 0).ToString();
            //string[] colnm = new string[] { "cc", "dt", "fn1", "fn2", "fid1", "fid2", "act", "pn1", "pn2", "rmk", "env", "pre" };
            //string[] colval = new string[] { cc, dt1.ToString(), fn1, fn2, fid1, fid2, act, pn1, pn2, rmk, env, un };
            //cl.DataRecordAddOnly("tbllog", colnm, colval);
        }

        public static void UpdLog(string cc, string fn1, string fn2, string fid1, string fid2, string act, string pn1, string pn2, string[] colnm, string[] colval, string un)
        {
            //DateTime dt1 = cl.GetTimeInSQLServer(), dt2 = DateTime.Now;
            //TimeSpan ts = dt2.Subtract(dt1);
            //string data = "", rmk = "";
            ...
            //{
            //    data += colnm[i].ToUpper() + ":" + colval[i].ToUpper() + "|";
            //}
            //if (data.Length > 4000) rmk = data.Substring(0, 4000);
            //else rmk = data;
            //string env = "APPVER:" + ver + "|";
            //env += "MACHINENAME:" + System.Environment.MachineName + "|";
            //env += "OSVER:" + System.Environment.OSVersion + "|";
            //env += "WINUSER:" + System.Environment.UserName + "|";
            //env += "TIMEDIFF(S):" + Math.Round(ts.TotalSeconds, 0).ToString();
            //string[] colnm2 = new string[] { "cc", "dt", "fn1", "fn2", "fid1", "fid2", "act", "pn1", "pn2", "qty", "up", "rmk", "env", "data", "pre" };
            //string[] colval2 = new string[] { cc, dt1.ToString(), fn1, fn2, fid1, fid2, act, pn1, pn2, qty.ToString(), up.ToString(), rmk, env, rmk, un };
            //cl.DataRecordAddOnly("tbllog", colnm2, colval2);
        }
    ...
    class CDataGridViewPrinter
    {
        ...
        private Font TheHeaderFont;
        private Color TheHeaderColor;
        ...
        public CDataGridViewPrinter(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage, string Title, Font aTitleFont, Color aTitleColor, string Header, Font HeaderFont, Color HeaderColor, bool WithPaging)
        {
            ...
            TheHeader = Header;
            TheHeaderFont = HeaderFont;
            TheHeaderColor = HeaderColor;
        ...
        private void DrawHeader(Graphics g)
        {
            ...
            {
                PageNumber++;
                string PageString = String.Format("{0:yyyy-MM-dd HH:mm:ss}", DateTime.Now) + " Page " + PageNumber.ToString();
                ...
                Font PageStringFont = new Font("Tahoma", 7, FontStyle.Regular, GraphicsUnit.Point);
            ...
            if (TheTitleText != "")
            {
                ...
                if (IsCenterOnPage) TitleFormat.Alignment = StringAlignment.Center;
                else TitleFormat.Alignment = StringAlignment.Near;
                string[] titles = TheTitleText.Split(new Char[] { '\n' });
                foreach (string title in titles)
                {
                    if (title.Trim() != "")
                    {
                        RectangleF TitleRectangle = new RectangleF((float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString(title, TheTitleFont).Height);
                        g.DrawString(title, TheTitleFont, new SolidBrush(TheTitleColor), TitleRectangle, TitleFormat);
                        CurrentY += g.MeasureString(title, TheTitleFont).Height;
            ...
            if (TheHeader != "")
            {
                ...
                string[] hdrs = TheHeader.Split(new Char[] { '\n' });
                foreach (string hdr in hdrs)
                {
                    if (hdr.Trim() != "")
                    {
                        RectangleF TitleRectangle = new RectangleF((float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString(hdr, TheHeaderFont).Height);
                        g.DrawString(hdr, TheHeaderFont, new SolidBrush(TheHeaderColor), TitleRectangle, TitleFormat);
                        CurrentY += g.MeasureString(hdr, TheHeaderFont).Height;
        ...
        public static void GetCrystalReportFileName(string cc, string bill)
        {
        }
        ...
        {
            if (Char.IsNumber(ch) || Char.IsControl(ch)) return true;
            else return false;
        }

        public static bool IsNum(char ch, string txt)
        {
            if (ch == (char)46)
            {
            ...
            else if (Char.IsNumber(ch) || Char.IsControl(ch)) return true;
            else return false;
        }
